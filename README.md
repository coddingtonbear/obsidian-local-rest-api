...
> **Deprecated:** The earlier header-driven PATCH format — `Operation`, `Target-Type`, `Target`, `Target-Scope`, etc. headers with the payload in the request body — is **deprecated and will be removed in 6.0**. The JSON instruction format shown above is the default; send `Markdown-Patch-Version: 1` to opt back into the legacy format (the same header also selects the legacy `::`-joined document map on GET). Responses served by the legacy format carry a `Deprecation: true; sunset-version="6.0"` header. To upgrade, drop that header and move each 1.x header into the JSON body (see the interactive docs for the mapping table).
