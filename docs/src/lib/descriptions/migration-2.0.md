...
- **Whitespace is spliced verbatim.** The 2.x engine inserts your `content` exactly as written at the edge of the target's span and adds no whitespace of its own; `Trim-Target-Whitespace` is gone. A leading `\n` in your content is what buys you a blank line before it — for `append` as much as for `prepend`. If your 1.x client relied on the server managing blank lines, re-check the output spacing.
