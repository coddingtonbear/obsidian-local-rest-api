...
- `file`: that file's NoteJson (the same shape `/search/` evaluates), or `null` for a folder, a deleted file, or an event with no file. `content` is included only when the filter mentions `content`.
...
The request body is an optional JsonLogic expression, evaluated against the `data` object above. It supports the same extra `glob` and `regexp` operators as `/search/`. Only events for which it is truthy are sent. With no body, or `{}`, every occurrence is sent. A filter JsonLogic can't evaluate is refused with `400`.
...
Anyone holding a signed stream URL sees the path and metadata of every event its filter matches, and note content if the filter mentions content, for as long as the stream stays open. The filter is part of the subscription, so the URL can't be used to widen it.
