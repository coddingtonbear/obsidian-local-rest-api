...
A signed URL authorizes a **whole-file** write to exactly the path it names. The signature covers the method, the normalized path and the expiry, and nothing else — so a request that also carries `Target-Type`/`Target` headers, or whose path continues into `/heading`, `/block` or `/frontmatter`, is refused with `40102` rather than quietly becoming a targeted edit of a document the link never named. Targeted writes need the API key.
