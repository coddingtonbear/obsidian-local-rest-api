  ...
  private registerTools(): void {
    ...
    this.tool(
      ...
      dedent`
        ...
        headings is a nested object mirroring the document's heading nesting: each heading's text maps to an object of its child headings, and a leaf heading maps to {} (e.g. {"Overview": {"Details": {}}}). To target a heading, use the path of keys from the top level down to it (e.g. ['Overview', 'Details']) as a vault_patch or vault_read heading target. A repeated sibling heading appears once, but its children still appear beneath that one key, because a heading is addressed by its whole path rather than its name — given '## Log / ### Monday' then '## Log / ### Tuesday', the tree is {"Log": {"Monday": {}, "Tuesday": {}}} and both are separately addressable. Only sections sharing an entire path are one address, resolving to the first in document order, so the tree lists exactly the headings you can target. blocks are bare reference IDs (no '^'); frontmatterFields are top-level key names. version is a content hash of the file — pass it back as vault_patch's ifMatch to make an edit conditional on the file being unchanged.
      `,
      { path: z.string().describe("File path relative to vault root") },
      READ_ONLY_ANNOTATIONS,
      async ({ path }: { path: string }) => {
